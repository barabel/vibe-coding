...
5. **You MUST invoke the `/tdd` skill** (via the Skill tool) to implement it. Do NOT skip this step and do NOT implement manually without the skill.
   Inside the skill, **SKIP the Planning step** — do NOT ask the user for confirmation, do NOT wait for approval. The issue file IS the specification. Go straight to the Tracer Bullet step.
