...
Аналог `loop.sh`, но Claude реализует issue через скилл `/tdd` (red→green→refactor). Поддерживает тот же флаг `--bp`.
...
bash scripts/claude-ralph/auto.sh имя-задачи 20 --tdd  # оба флага
```
